 ...
 */
class AspModelGenerator extends BaseCommand
{
    use GeneratorTrait;
     ...
     * @var string
     */
    protected $usage = 'make:app-model <name> [options]';
    ...
    public function run(array $params)
    {
        ...
        $this->template  = 'app.model.tpl.php';
    ...
    protected function prepare(string $class): string
    {
        ...
        $asc     = $this->getOption('asc');
        $ds      = $this->getOption('dataset');
        $allowedFields = '';
        $orderAsc = 'false';
        $dataSet = 'false';
        $class_namespace = trim($class, '\\');
        $class_namespace = trim(str_replace('/', '\\', substr($class_namespace, 0, -1 * strlen(basename($class_namespace)))), '\\');

        if ($fields) {
            $bufArr = explode(',', $fields);
            if ($this->template !== 'model.tpl.php') {
                foreach ($bufArr as $field) { $allowedFields .= "'" . trim($field) . "',"; }
                if ($allowedFields !== '') { $allowedFields = substr($allowedFields, 0, -1); }
            }
        } else if ($this->template == 'app.model.tpl.php') {
            $fields = '*';
        }
        
        if ($ds && $ds !== 'false') { $dataSet = 'true'; }

        if ($order && $asc && $asc !== 'false') { $dataSet = 'true'; }
        ...
        return $this->parseTemplate($class, 
            ['{table}', '{return}', '{fields}', '{allowedFields}', '{order}', '{orderAsc}', '{dataSet}', '{class_namespace}'], 
            [$table, $return, $fields, $allowedFields, $order, $orderAsc, $dataSet, $class_namespace]
        );
    }
}
