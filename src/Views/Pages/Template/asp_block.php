    ...
    <div class="card">
        ...
        <div class="card-body">
                        ...
                        <table class="table table-striped table-hover data-tables no-footer" role="grid">
                            ...
                            <tbody asp-tmpl-get="tbody">
                                <tr role="row" class="items" data-id="{{id}}">
                                    ...
                                    <td editable="text" i-name="name" i-pattern="[a-zA-Z0-9-_]" i-maxlength="63">{{name}}</td>
            ...
            <?php
                ...
                //echo command('make:app-controller News --model News --force');
                //echo command('make:app-model-direct News --table article --namespace Sygecon\AdminBundle --suffix --force');
                //echo command('make:app-model News --table article --suffix --force');
            ?>
        </div>
        <!-- /.card-body -->
            ...
            <template>
                <form id="form-edit-block" class="p-1" enctype="multipart/form-data">
                    <div class="form-group pt-3">
                        ...
                        <input type="text" class="form-control mt-1" name="name" placeholder="<?= lang('Admin.global.name') ?>" i-pattern="[a-z0-9-_]" maxlength="64" required>
                    </div>
                    <div class="form-group pt-3">
                        <label><?= lang('HeadLines.layout.typeLabel') ?></label>
                        <select title="<?= lang('HeadLines.layout.typeTitle') ?>" name="type" class="form-select form-control mt-1">
                            <?php if (isset($typeList)) : ?>
                                <?php foreach ($typeList as $item) : ?>
                                    <option value="<?= $item ?>" <?php if ($item === 'php') echo 'selected="selected"'; ?>> <?= strtoupper($item) ?></option>
                                <?php endforeach; ?>
                            <?php endif ?>
                        </select>
                    </div>
